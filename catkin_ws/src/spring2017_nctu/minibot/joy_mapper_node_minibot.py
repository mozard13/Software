...
class JoyMapper(object):
    ...
    def cbJoy(self, joy_msg):
        self.joy = joy_msg
        self.publishControl(joy_msg)
        self.processButtons(joy_msg)

    def publishControl(self, joy_msg):
        ...
        if (self.joy.axes[1] > 0):
            ...
            car_cmd_msg.v = joy_msg.buttons[0] * -self.v_gain #Left stick V-axis. Up is positive
        elif (joy_msg.buttons[3] == 1): 
            car_cmd_msg.v = joy_msg.buttons[3] * self.v_gain
        else:
            car_cmd_msg.v = 0
        if self.bicycle_kinematics:
            ...
            if (self.joy.axs[3] > 0) :
                ...
                steering_angle = joy_msg.buttons[1] * self.steer_angle_gain
            elif (joy_msg.buttons[2] == 1):
                steering_angle = joy_msg.buttons[2] * -self.steer_angle_gain
            else:
                steering_angle = 0
            ...
            if (self.joy.axes[3] > 0):
                ...
                car_cmd_msg.omega = joy_msg.buttons[1] * self.omega_gain
            elif (joy_msg.buttons[2] ==1):
                car_cmd_msg.omega = joy_msg.buttons[2] * -self.omega_gain
            else:
                car_cmd_msg.omega = 0
        print(self.joy.axes[1])
        #print('X: {0}, Y: {1}, A: {2}, B: {3}'.format(joy_msg.buttons[2],joy_msg.buttons[3],joy_msg.buttons[0],joy_msg.buttons[1]))
...
# logitek = 8, left joy = 9, right joy = 10
    ...
    def processButtons(self, joy_msg):
        ...
        elif (joy_msg.buttons[9] == 1): #the start button
            override_msg = BoolStamped()
            override_msg.header.stamp = self.joy.header.stamp
        ...
        elif (joy_msg.buttons[7] == 1): #push left joystick button 
